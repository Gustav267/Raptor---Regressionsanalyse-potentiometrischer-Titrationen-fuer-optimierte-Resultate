...
df = pd.read_excel("\\Potentiometrie-all-data.xlsx")
...
result = model.fit(ph_wert, params, x=x_scaled, method="leastsq", max_nfev=iterationen)
...
## Tangenten- und pH-Verfahren entfernt. Auswertung erfolgt nur über
## die Nullstelle der zweiten Ableitung der Fit-Kurve.

### Nullstelle Zweiter Ableitung###

# Zweite Ableitung der Fit-Kurve berechnen und Nullstellen suchen
# Suche im gesamten Messbereich (vol_min, vol_max) mit Brentq
x_scaled_nullstellen_zweiter_ableitung = root_scalar(
    lambda x: zwe_abl(x, **result.best_values),
    bracket=(vol_min+1, vol_max-1),
    method='brentq'
).root
...
ax1.plot(x_linespace_ml, ph_zu_linespace, color='orange', label='Regression', linewidth=1.2, zorder=4)
#ax2.plot(x_linespace_ml, ph_fit_deriv, color='green', label='1. Ableitung', linewidth=0.5, linestyle='dashed', zorder=1)
#ax2.plot(x_linespace_ml, ph_fit_deriv2, color='gray', label='2. Ableitung', linewidth=0.5, linestyle=':', zorder=1)
...
ax1.scatter(vol_nullstellen_zweiter_ableitung, seven_pl(x_scaled_nullstellen_zweiter_ableitung, **result.best_values), color='purple', label=f'f\'\'(x)=0 bei {vol_nullstellen_zweiter_ableitung:.1f} ml', marker='_', zorder=9, s=20)  
